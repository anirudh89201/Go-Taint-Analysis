...
// Classify vulnerabilities based on source and sink combinations
func classifyVulnerability(src, sink string) string {
	if strings.Contains(src, "userInput") && strings.Contains(sink, "SQLQuery") {
		return "SQL Injection Vulnerability"
	}
	if strings.Contains(src, "userInput") && strings.Contains(sink, "ShellExec") {
		return "Command Injection Vulnerability"
	}
	if strings.Contains(src, "userInput") && strings.Contains(sink, "HTMLResponse") {
		return "Cross-Site Scripting (XSS) Vulnerability"
	}
	if strings.Contains(src, "userInput") && strings.Contains(sink, "FileWrite") {
...
func main() {
	...
	err := filepath.Walk(location, func(path string, info os.FileInfo, walkErr error) error {
		...
		if info.Name() == "main.go" {
			...
			detectedFlows := make(map[string]string)
			...
			for _, src := range sources {
				for _, sink := range sinks {
					...
					for _, line := range lines {
						if strings.HasPrefix(line, "Tainted flow detected") {
							key := fmt.Sprintf("%s -> %s", src, sink)
							detectedFlows[key] = line
						}
					}
				}
			}

			for key, line := range detectedFlows {
				parts := strings.Split(key, " -> ")
				src := parts[0]
				sink := parts[1]
				vuln := classifyVulnerability(src, sink)
				resultsWriter.WriteString(fmt.Sprintf("%s\nVulnerability: %s\n\n", line, vuln))
			}
